...
class Platform(object):
    ...
    def rankItems(self, mode='random'):
        ...
        if mode == 'random':
            ...
            ranking = sorted(
                self.items.keys(),
                key=
                lambda x: confidenceBound(self.items[x].getUpVotes(), self.items[x].getViews(), self.num_user)[0],
                reverse=True)
        elif mode == 'ucb':
            ranking = sorted(
                self.items.keys(),
                key=
                lambda x: confidenceBound(self.items[x].getUpVotes(), self.items[x].getViews(), self.num_user)[1],
                reverse=True)
        else:
...
def confidenceBound(ups, views, T):
    if views == 0:
        return (9999, 9999)
    c = 1
    p = ups / views
