...
def topKinK(itms, K, final_order=None, rank_std="random"):
    itms_final = np.copy(itms)  # a copy of the final list
    exp_order = sorted(
        range(len(itms_final)),
        key=lambda x: itms_final[x].getQuality(),
        reverse=True)
    ...
    if rank_std == "random":
        ...
        final_order = sorted(
            range(len(itms_final)),
            key=lambda x: itms_final[x].getUpVotes(),
            reverse=True)
...
def printPerfmeas(platform, num_user, K):
    ...
    ktd = kendallTauDist(
        final_list, final_placements=final_ranking, rank_std="quality")
    print()
    ...
    #***** Top K Percentage
    topK = topKinK(
        final_list, K=K, final_order=platform.itemRanking, rank_std="random")
    print()
    print("Percentage of top", K, "items that are actually in top ", K, ":",
          topK['percent'])
...
def plotKDT(plt, ktds, rankMode):
    plt.plot(ktds, label='rank by %s' % (rankMode))


def plotTopK(plt, topKs, rankMode, K):
    plt.plot(topKs, label='rank by %s' % (rankMode))


def plotHappiness(plt, happy, rankMode):
